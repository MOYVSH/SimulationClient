...
namespace MOYV.UGUI
{
    ...
    /// messageReceiver或者GameViewInputManager下面的Sprite会接收到的事件
    /// 
    ...
    /// 
    /// </summary>
    public class GameViewInputManager : MonoBehaviour
    {
        public class TouchEvent
        {
            public Vector3 pressPosition; //world position
            public Vector3 position; //world position
            public Vector3 delta; //world position delta
        ...
        void Update()
        {
            bool flag = !(isCheckOnUGUI && InputTool.CheckMouseOnUI());
        ...
        void OnTouchDown()
        {
            ...
            {
                pressPosition = raycastCamera.ScreenToWorldPoint(Input.mousePosition)
            };
        ...
        void OnTouchUp()
        {
            ...
            if (m_touchTarget != null)
            {
                ...
                if (Time.realtimeSinceStartup - m_touchTimeDelta < clickDelta &&
                    m_event.pressObject == m_event.pointerOnObject)
                {
        ...
        void OnTouchMove()
        {
            ...
            m_event.delta = pos - m_event.position;
