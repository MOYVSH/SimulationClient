namespace MOYV.UGUI
{
    ...
    public class SpriteDrag : MonoBehaviour
    {
        ...
        [Tooltip("拖动的对象，默认为自己.")] public Transform dragTarget = null;

        [Tooltip("如果为null，则使用mainCamera.")] public Camera rayCastCamera = null;

        [Tooltip("射线检测的Layer")] public LayerMask dragRayCastMask = -1;
        public LayerMask dropRayCastMask = -1;
        public bool dragCheckUGUI = true; //drag时是否判断在ugui上

        [Tooltip("判断drag时是否忽略上面")] public bool dragIgnoreTop = true;
        [Tooltip("判断drop时是否忽略下面")] public bool dropIgnoreBottom = true;

        [Header("Drag Setting")] [Tooltip("在拖动时是否固定在拖动物的原点.")]
        public bool isDragOriginPoint = false;

        [Tooltip("当isDragOriginPoint为true时，拖动时的偏移值.")]
        public Vector2 dragOffset;

        [Tooltip("主要用于影响层级显示.")] public float dragOffsetZ = 0f;

        [Tooltip("Drag时的变化的大小.")] public float dragChangeScale = 1f;

        [Tooltip("Drag时角度的变化值")] public float dragChangeRotate = 0f;

        [Tooltip("拖动的时候在哪个层.没有设置的话为当前Sort Layer")]
        public string dragSortLayerName;

        [Tooltip("拖动时变化的层级数")] public int dragChangeOrder = 0;

        [Tooltip("触发的原点，默认为当前对象")] public Transform triggerPos;

        [Tooltip("触发的类型")] public TriggerType triggerType = TriggerType.Point;

        [Tooltip("当触发类型为圆时,设置半径")] public float triggerRadius = 0.5f;

        [Tooltip("当触发类型为范围时,设置宽高")] public Vector2 triggerRange = new Vector2(0.5f, 0.5f);

        //要发送的事件名字
        [Header("Event")] public bool sendHoverEvent = false;
        ...
        [Header("Back Effect")] [Tooltip("释放时，是否自动返回")]
        public bool releaseAutoBack = false;

        [Tooltip("返回时的效果")] public DragBackEffect backEffect = DragBackEffect.None;
        [Tooltip("效果时间")] public float backDuring = 0.5f;
        [Tooltip("Tween 的效果")] public Ease tweenEase = Ease.Linear;
        ...
        void Update()
        {
            ...
            if (Input.touchCount < 2)
            {
                ...
                {
                    if (!m_isDown && !InputTool.isOnUI)
                    {
                        if (dragCheckUGUI && InputTool.CheckMouseOnUI()) return;

                        RaycastHit2D[] hits = Physics2D.RaycastAll(
                            rayCastCamera.ScreenToWorldPoint(Input.mousePosition), Vector2.zero, 0, dragRayCastMask);
        ...
        void OnMouseBeginDrag()
        {
            ...
            {
                dragTarget.DOLocalRotate(m_cacheRotation + new Vector3(0f, 0f, dragChangeRotate), 0.4f,
                    RotateMode.Fast);
            }
            ...
            {
                m_dragOffset = dragTarget.position -
                               rayCastCamera.ScreenToWorldPoint(new Vector3(Input.mousePosition.x,
                                   Input.mousePosition.y, m_screenPosition.z));
            }
                ...
                if (cols != null && cols.Length > 0)
                {
                    foreach (Collider2D col in cols)
                    {
                        if (col.gameObject != gameObject)
                            col.SendMessage(onHoverMethodName, dragTarget.gameObject,
        ...
        void OnMouseUpHandler()
        {
            ...
            if (sendHoverEvent)
            {
                ...
                if (cols != null && cols.Length > 0)
                {
                    ...
                    {
                        if (col.gameObject != gameObject)
                            col.SendMessage(onDropMethodName, dragTarget.gameObject,
                                SendMessageOptions.DontRequireReceiver);
                        if (dropIgnoreBottom) break;
                    }
        ...
        public void BackPosition()
        {
            ...
            switch (backEffect)
            {
                ...
                case DragBackEffect.TweenPosition:
                    ...
                    dragTarget.DOMove(new Vector3(m_cachePosition.x, m_cachePosition.y, dragTarget.position.z),
                        backDuring).SetEase(tweenEase).OnComplete(() =>
                    {
                        this.enabled = true;
                        this.m_canDrag = true;
                        dragTarget.position = m_cachePosition;
