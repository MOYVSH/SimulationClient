Shader "UI/Procedural UI Image"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			half visible(float2 pos,float4 radius,float2 wh){
				half4 p = half4(pos,wh.x-pos.x,wh.y-pos.y);
				half v = min(min(min(p.x,p.y),p.z),p.w);
				bool4 b = bool4(all(p.xw<radius[0]),all(p.zw<radius[1]),all(p.zy<radius[2]),all(p.xy<radius[3]));
				half4 vis = radius-half4(length(p.xw-radius[0]),length(p.zw-radius[1]),length(p.zy-radius[2]),length(p.xy-radius[3]));
