...
public abstract class RecycleView<T> : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler where T : RecycleViewItem
{
    ...
    public int pageCount = 0; // 一屏显示几个Cell 现在只在跳转用 可以不填

    protected Action<T, int> FuncCallBackFunc;
    protected Action<T, int> FuncOnClickCallBack;
    protected Action<int, bool, T> FuncOnButtonClickCallBack;
    ...
    public virtual void Init(Action<T, int> callBack)
    {
        Init(callBack, null);
    }

    public virtual void Init(Action<T, int> callBack, Action<T, int> onClickCallBack,
        Action<int, bool, T> onButtonClickCallBack)
    {
    ...
    public virtual void Init(Action<T, int> callBack, Action<T, int> onClickCallBack)
    {
        ...
        if (cell == null)
        {
            cell = content.transform.GetChild(0).GetComponent<T>();
        }

        // ////////////////////** Cell 处理 **////////////////////
        // m_CellGameObject.transform.SetParent(m_Content.transform.parent, false);
        SetPoolsObj(Instantiate<T>(cell));
    ...
    public virtual void ShowList(int num , Action onComplet = null)
    {
        minIndex = -1;
        maxIndex = -1;
    ...
    /// </summary>
    /// <param name="index">索引ID</param>
    public void MoveToCell(int index,Action callback = null)
    {
        if (_tweener != null)
        {
            _tweener.Kill();
        ...
        if (lines > 1)
        {
            ...
            newPos = cellInfos[theFirstIndex].pos;
        }
        else
        {
            int cellInfosLength = cellInfos.Length;
            if (cellInfosLength > pageCount)
            {
                newPos = cellInfosLength - index < pageCount
                    ? cellInfos[cellInfosLength - pageCount].pos
                    : cellInfos[index].pos;
            }
            else
            {
        ...
        if (dir == E_Direction.Vertical)
        {
            ...
            if (lines > 1)
            {
                if (index >= cellInfos.Length - (cellInfos.Length % lines)) // 判断是否为最后一排
                {
                    posY = index <= lines ? -newPos.y - paddingTop : -newPos.y - col + cellH * 2 / 3;
                    isLast = true;
                }
                else
                {
                    posY = index <= lines ? -newPos.y - paddingTop : -newPos.y - col;
                }
            }
            else
            {
            ...
            if (isLast)
            {
                _tweener = DOVirtual.Float(scrollRect.verticalNormalizedPosition,0,0.5f, (value) =>
                {
            ...
            {
                _tweener = contentRectTrans.DOAnchorPosY(posY, 0.5f);
            }
        ...
        else
        {
            ...
            // index>lines,位置应该为 (x+row,y)
            var posX = index <= lines ? -newPos.x + paddingLeft : -newPos.x + row;
            _tweener = contentRectTrans.DOAnchorPosX(posX, 0.5f); //contentRectTrans.anchoredPosition = new Vector2(posX, contentRectTrans.anchoredPosition.y);
            _tweener.onComplete = () =>
            {
                callback?.Invoke();
            };
    ...
    //取出 cell
    protected virtual T GetPoolsObj()
    {
        T cell = null;
        if (Pool.Count > 0) cell = Pool.Pop();
        if (cell == null) cell = Instantiate(this.cell) as T;
    ...
    //存入 cell
    protected virtual void SetPoolsObj(T cell)
    {
        if (cell != null)
        {
            Pool.Push(cell);
            SetActive(cell.gameObject, false);
            cell.gameObject.name = "temp";
        }
    }

    //回调
    protected void Func(Action<T, int> func, T selectObject, bool isUpdate = false)
    {
    ...
    public T GetCellGameObject(int index)
    {
        // 为了保证拿到正确数据，根据index应该-1拿到正确数据
        return cellInfos[index].obj;
    }

    public int GetCellIndex(T obj)
    {
        // 第0号是模板，所以列表中索引应该-1
        return Convert.ToInt32(obj.name);
    }
