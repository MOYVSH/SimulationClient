...
- [ChunkSystem.cs](file://Assets/Game/Scripts/MiniGame_Scripts/System/ChunkSystem.cs)
</cite>

## 更新摘要
**变更内容**   
- 新增 CoordinateUtility 坐标工具类，支持负数处理的完整坐标转换功能
- 完善 WorldData 权威数据层实现，提供实体管理和事件通知机制
- 重构 ChunkSystem 分区管理系统，实现 3x3 激活窗口管理
- 增强坐标变换工具集，支持多坐标系间的高效转换
...
**更新** 本次更新重点增强了坐标系统、权威数据层和分区管理的实现质量，特别是引入了完善的 CoordinateUtility 工具类和改进的 ChunkSystem 管理机制。
...
- 分区管理：Chunk/ChunkManager/ChunkSystem
...
- 坐标系与工具
  ...
  - **新增** CoordinateUtility 提供完整的坐标转换工具，正确处理负数边界情况。
- WorldGrid 空间数据库
  - Cell 记录 TreeId/BuildingId/RoadId/Flags；提供 IsWalkable/IsOccupied/GetEntityAt 等查询。
- WorldData 权威数据层
  - **增强** 以 ID 索引 Trees/Buildings/Roads/Workers，并提供增删改事件，供各系统订阅。
- ChunkSystem 分区管理
  - **重构** 维护 3×3 激活窗口，触发 ChunkActivated/ChunkDeactivated 事件。
- Terrain 初始同步
...
### 坐标系统与工具（CoordinateUtility）
- 职责边界
  - 提供 WorldPos、GridPos、ChunkPos 之间的零分配双向转换。
  - 正确处理负数坐标和边界检查，确保坐标转换的数学正确性。
- 关键能力
  - 坐标类型转换、边界验证、距离计算、包围盒检测。
  - 支持浮点和整数坐标的精确转换。
- 验收标准
  - 负数坐标正确处理、边界外坐标安全返回、转换精度无损失。

```mermaid
classDiagram
class CoordinateUtility {
+WorldToGrid(worldPos) GridPos
+GridToWorld(gridPos) WorldPos
+GridToChunk(gridPos) ChunkPos
+ChunkToGrid(chunkPos) GridPos
...
CoordinateUtility --> WorldPos : "转换"
CoordinateUtility --> GridPos : "转换"
CoordinateUtility --> ChunkPos : "转换"
```
...
### 世界网格与分区（WorldGrid + ChunkSystem）
- 职责边界
  - WorldGrid：空间查询、占用管理、可行走判断。
  - ChunkSystem：3×3 激活窗口，事件驱动下游系统按需加载/卸载。
- 数据结构
...
WorldGrid --> Cell : "持有"
ChunkSystem --> WorldGrid : "查询占用/可行走"
```
...
- 组件耦合与内聚
  ...
  - ChunkSystem 与 View 系统松耦合（通过事件驱动）。
  - A* 适配层对 WorldGrid 有直接依赖，但封装了插件细节。
  - **新增** CoordinateUtility 作为独立工具层，被多个组件复用。
- 外部依赖
...
CS["ChunkSystem"] --> WG
CS --> TV["TreeViewSystem"]
CS --> BV["BuildingViewManager"]
CU["CoordinateUtility"] --> WG
...
- 优化策略
  ...
  - **新增** 坐标转换优化：CoordinateUtility 采用零分配算法，减少 GC 压力。
- 验证方式
...
## 故障排查指南
- 常见问题定位
  - 坐标转换异常：检查 GridPos/ChunkPos/WorldPos 的双向转换与边界处理，特别关注负数坐标。
...
## 结论
本方案以 WorldGrid 为核心，结合 WorldData 权威数据层与 ChunkSystem 分区管理，构建了可扩展、高性能的世界网格系统。通过数据与表现分离、对象池与局部更新、任务就近分配与 Tick 统一管理，能够在 10000+ 树木与 100+ 工人的规模下稳定运行。本次更新特别增强了 CoordinateUtility 坐标工具的健壮性和 ChunkSystem 的分区管理效率，确保了负数坐标的正确处理和更高效的激活窗口管理。后续优化聚焦于 LOD、路径缓存、GC 与 GPU 批处理，确保达到 CPU/GPU 预算。
